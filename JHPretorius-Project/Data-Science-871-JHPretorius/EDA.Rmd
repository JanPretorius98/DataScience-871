...
By using my Google Maps API, I was able to geocode the locations of every profile in the data. The goal is to visualise how location effects profile views. The code chunk below was used to geocode the data.
```{r}
# Note: ommented out, due to costs associated with geocoding through the API
# df_city <- df %>% 
...
```
####    Figure 3: Geographic data visualisation of profile views. Size and colour of bubbles in top panel indicate profile views. Colour of country in the bottom panel indicate profile views.
```{r warning=FALSE, fig.align='center',  fig.height= 8, fig.width=7.5, echo=FALSE}
...
####    Figure 4: Lollipop chart of number of users by country. The colour of the lollipops indicate mean profile views.
...
# Produce a Ridgline plot to look at how profile views are distributed for flirtInterests_chat, flirtInterests_friends, flirtInterests_date & isMobile 

```


#### Figure 4: Bubble plot of number of pictures in profile and profile views. A non-linear model (loess method) was once again fitted on the plot to discern possible patterns and differences between bios with emojis and those without. Size of dots present how detailed the account is.
