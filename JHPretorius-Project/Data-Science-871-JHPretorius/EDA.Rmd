...
pacman::p_load(dplyr, ggplot2, tidyverse, rsample, caret, glmnet, vip, pdp, stringr, 
               ...
               randomForest, gt, knitr, kableExtra, cowplot, textdata)
...
data <- data %>% 
  select(c("genderLooking","age", "counts_details", "counts_pictures", "counts_profileVisits", "counts_kisses", "flirtInterests_chat", "verified", "lang_count", "lang_de", "whazzup"))
...
# Create a data frame
data <- data.frame(
  Variable = c("genderLooking", "age", "counts_details", "counts_pictures", "counts_profileVisits", "counts_kisses", "flirtInterests_*", "verified", "lang_count", "lang_*", "whazzup"),
  Description = c("Preferred gender the subject is looking to engage with. Represented as 'M' for male, 'F' for female, 'both' for male and female, or 'none'.",
                  ...
                  "A phrase that represents the profile's 'bio'.")
) %>% tibble::as_tibble()
...
Figure 3 below aims to present whether there is a difference in the distribution of likes received based on the newly created dummy variables, `has_emoji`, `contains_popular_word`, and `night_owl`. There seem to be some slight differences in likes received, supporting the idea that the use of emojis and certain words do suggest higher levels of trust. Interestingly, being online seems to be negatively associated with profile views as is shown in the right panel of figure 3. However, being online during night time also may increase profile views, but I view this variable more as a control variable, rather than a causal one, as more people tend to be online during night time than in day time.
...
df$whazzup[is.na(df$whazzup)] <- ""

# Add a new column for bio length
df <- df %>%
  mutate(bio_length = str_length(whazzup))

# Density plot of bio lengths
densplot <- ggplot(df, aes(x=bio_length)) +
  geom_density(fill="#1beaa7", alpha=0.8, color = "white") +
  labs(x = "Bio Length", y = "Density") +
  th
```
...
```{r fig4, warning=FALSE, fig.align='center', echo=FALSE}
